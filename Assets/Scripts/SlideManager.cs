...
public class SlideManager : MonoBehaviour
{
    ...
    private GameObject[,] preGridObjects;
    private GameObject[,] preOccupants;
    ...
    public IEnumerator SlideBlocks(List<(int x, int y)>[] insertedBlocks)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            preGridObjects = (GameObject[,])boardData.gridObjects.Clone();
            preOccupants = (GameObject[,])boardData.occupants.Clone();

            foreach ((int col, int row) in insertedBlocks[i])
            {
                var moveList = new List<(GameObject obj, Vector3 from, Vector3 to)>();
    ...
    private void SetBoardData(int data, GameObject obj, GameObject occ, int x, int y)
    {
